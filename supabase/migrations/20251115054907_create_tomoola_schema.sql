...
-- Index on folk_dances table to quickly find dances by ID
CREATE INDEX IF NOT EXISTS idx_folk_dances_id ON folk_dances(id);

-- If you want to link bookings to a folk dance, first add a column:
ALTER TABLE bookings ADD COLUMN folk_dance_id uuid REFERENCES folk_dances(id);

-- Then create an index on that column:
CREATE INDEX IF NOT EXISTS idx_bookings_folk_dance ON bookings(folk_dance_id);
